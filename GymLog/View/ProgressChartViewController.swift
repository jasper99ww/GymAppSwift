...
class ProgressChartViewController: UIViewController, ChartViewDelegate {
    ...
    lazy var presenter = ChartPresenter(chartPresenterDelegate: self)
    ...
    var retrievedAllWorkouts: [String: [DoneWorkoutInformation]] = [:]
    ...
    override func viewDidLoad() {
        ...
        presenter.getDataByVolume(titles: presenter.arrayOfWorkoutTitles)
    ...
    @IBAction func segmentedControlChanged(_ sender: UISegmentedControl) {
        if sender.selectedSegmentIndex == 0 {
            segmentedControlForWeek(period: "week")
                    ...
                    presenter.segmentedControlSelect(selectedIndex: 0)
//            segmentedControlForWeek(period: "month")
    ...
    func setData2(entries: [LineChartDataSet]) {
    ...
    // 1
    // Done Workout Information
    func findTrainingVolume(ex: [String], period: String) {
        
        let group2 = DispatchGroup()
     
        var new: [String: [DoneWorkoutInformation]] = [:]
        ...
        lineChart.xAxis.valueFormatter = ChartXAxisFormatter(referenceTimeInterval: referenceTimeInterval, dateFormatter: dateFormatter)
            ...
            for (key, value) in new {
                ...
                for values in value {
                    ...
                    let highlightedValue = HighlightedExercise(sets: 0, reps: values.reps, time: values.time)
    ...
    func findMaxValueWithSelection(type: String, period: String) {
        ...
        for (key, value) in retrievedExerciseMaxWithSelection {
            ...
            for values in value {
                ...
                let maxWeight = values.weight
                let volume = Double(values.volume)
                let highlightedValue = HighlightedExercise(sets: values.sets, reps: values.reps, time: "")
...
func retrieveDocumentsArray() {
    ...
    //MARK: -SEGUES NIEWAZNEEeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
...
//        self.lineChart.animate(xAxisDuration: 0.04 * Double(self.lineChartEntry1.count), easingOption: .linear)
        guard let lineChartData = lineChart.data?.entryCount else { return }
        self.lineChart.animate(xAxisDuration: 0.02 * Double(lineChartData), easingOption: .linear)
    }
...
extension ProgressChartViewController: ChartPresenterDelegate {
    
    func setData(dataSet: [LineChartDataSet]) {
        setData2(entries: dataSet)
        print("setted")
    }
    
    func setAxisFormatter(axisFormatter: ChartXAxisFormatter) {
        lineChart.xAxis.valueFormatter = axisFormatter
    }
}
